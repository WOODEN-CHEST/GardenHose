...
internal class DropDownList<OptionType> : ConnectorElement
{
    ...
    private void UpdateButtonAbility()
    {
        for (int i = 0; i < _options!.Length; i++)
        {
            ...
            _options[i].Button.IsEnabled = IsButtonEnabled;
    ...
    {
        _glow.Mask = FloatColor.InterpolateRGB(UnselectedColor, 
            FloatColor.InterpolateRGB(s_hoverColor, DefaultClickedColor, _clickStrength), _hoverStrength);
        _text.Mask = FloatColor.InterpolateRGB(FloatColor.White, 
            FloatColor.InterpolateRGB(s_hoverColorText, DefaultClickedColor, _clickStrength), _hoverStrength);
